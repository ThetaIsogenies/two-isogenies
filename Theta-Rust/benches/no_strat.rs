...
// version, no strategy is used for the isogeny chain and a quadratic number of
// doublings are computed and only a linear number of images. As images are
// cheaper than doublings for our chain, this is particularly ineffective.

#![allow(non_snake_case)]

use criterion::{criterion_group, criterion_main, Criterion};
use std::time::Duration;
use theta_rs::theta381::{
    product_isogeny_no_strategy, CouplePoint, Curve, EllipticProduct, Fq, Point,
};

fn criterion_no_strategy_product_chain(c: &mut Criterion) {
    let A1 = Fq::ZERO;
    ...
    c.bench_function(
        "Product isogeny of 208 steps without optimised strat with 381-bit prime",
        |b| b.iter(|| product_isogeny_no_strategy(&E1E2, &P1P2, &Q1Q2, &image_points, n)),
    );
}
...
criterion_group! {
    ...
    targets = criterion_no_strategy_product_chain
}
